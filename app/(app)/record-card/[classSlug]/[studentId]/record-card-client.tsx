  ...
  return (
    <div className="px-8 py-10">
      ...
      <div className="no-print mx-auto flex max-w-4xl items-center justify-between pb-4">
        {previousStudent ? (
          <Button href={`/record-card/${classRow.slug}/${previousStudent.id}`} variant="secondary" size="sm">
            ← {previousStudent.name}
          </Button>
        ) : (
          <span />
        )}
        <Button href={`/record-card/${classRow.slug}`} variant="secondary" size="sm">
          All students
        </Button>
        {nextStudent ? (
          <Button href={`/record-card/${classRow.slug}/${nextStudent.id}`} variant="secondary" size="sm">
            {nextStudent.name} →
          </Button>
        ) : (
          <span />
