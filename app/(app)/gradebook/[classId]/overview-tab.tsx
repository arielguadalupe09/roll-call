...
export default function OverviewTab({
  ...
  const recitationByStudent = useMemo(() => {
    ...
    for (const log of recitationLogs) {
      ...
      const entry = map.get(log.student_id) ?? { sum: 0, count: 0 };
      entry.sum += log.score;
      entry.count += 1;
            ...
            return (
              <tr key={s.id} className="bg-white">
                ...
                {midtermExam && (
                  <Cell>
                    {(() => {
                      const score = majorExamScoreByKey.get(`${midtermExam.id}_${s.id}`)?.score;
                ...
                <Cell>
                  {recitation && recitation.count > 0
                    ? `${(recitation.sum / recitation.count).toFixed(1)}/5`
                    : null}
                </Cell>
              </tr>
            );
