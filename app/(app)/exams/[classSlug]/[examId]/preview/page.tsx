...
export default async function ExamPreviewPage({
  ...
  const { data: classRowData } = await supabase.from("classes").select("*").eq("slug", classSlug).single();
  if (!classRowData) notFound();
    ...
    <div className="px-8 py-10 print:px-0 print:py-0">
      <div className="mx-auto max-w-3xl">
        <div className="flex items-center justify-between print:hidden">
          <Button href={`/exams/${classSlug}/${examId}`} variant="secondary" size="sm">
            ← Back to editing
          </Button>
