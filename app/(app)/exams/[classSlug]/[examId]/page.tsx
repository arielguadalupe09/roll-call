...
export default async function ExamBuilderPage({
  ...
  params: Promise<{ classSlug: string; examId: string }>;
}) {
  const { classSlug, examId } = await params;
  const supabase = await createClient();

  const { data: classRow } = await supabase.from("classes").select("id").eq("slug", classSlug).single();
    ...
    <div className="px-8 py-10">
      <div className="mx-auto max-w-3xl">
        <div className="flex items-center justify-between">
          <Button
            href={`/gradebook/${classSlug}?tab=${GRADEBOOK_TAB_FOR_KIND[(exam as Exam).kind]}`}
            variant="secondary"
            size="sm"
          >
            ← Back to gradebook
          </Button>
          <Button href={`/exams/${classSlug}/${examId}/preview`} variant="secondary" size="sm">
            Preview exam →
          </Button>
        </div>
